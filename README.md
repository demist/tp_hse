...
**Оценка за тесты** = *Средний балл*(По всем 12 тестам); Каждый тест оценивается по 10-балльной шкале. 
Оценка за тесты не округляется.

**Финальная оценка** = *Математическое округление* (0.1\*`Тесты` + 0.15\*`ТЗ1` + 0.15\*`ТЗ2` + 0.15\*`ТЗ3` + 0.45\*`ЭКЗ`)
